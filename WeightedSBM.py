...
def generate_noisy_sbm(sbm, G, noise_type='element', noise_density=0.05, noise_magnitude=0.1, random_state=None):
    """
    Generate a noisy version of a stochastic blockmodel.
    ...
    G : networkx.Graph
        The original graph generated by the SBM
    noise_type : str, optional
        Type of noise to add: 'element', 'column', or 'sparse'
        'element': Original noise on individual elements
        'column': Perturb entire columns/rows simultaneously
        'sparse': Add sparse random noise
        Default is 'element'
    noise_density : float, optional
        Density of the noise matrix
        Default is 0.05 (5% of elements)
    noise_magnitude : float, optional
        Maximum absolute magnitude of noise
        Default is 0.1
    random_state : int or numpy.random.RandomState, optional
        Random state for reproducibility
